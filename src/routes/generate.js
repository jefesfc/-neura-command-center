...
router.post('/', async (req, res) => {
  const { brief, system, format = '1:1', tone = 'profesional' } = req.body;
  ...
  // Run pipeline async
  runPipeline(jobId, { brief, system, format, tone });
});
...
router.get('/stream/:jobId', (req, res) => {
  ...
  // Poll job state every 300ms and push updates
  let lastStepsSent = {};
...
router.post('/step', async (req, res) => {
  const { postId, step, brief, system, format, imagePrompt } = req.body;
  ...
  try {
    if (step === 'copy') {
      ...
      await query(
        'UPDATE posts SET headline=$1, bullets=$2, cta=$3, updated_at=NOW() WHERE id=$4',
    ...
    if (step === 'caption') {
      ...
      await query('UPDATE posts SET caption=$1, hashtags=$2, updated_at=NOW() WHERE id=$3', [caption.caption, caption.hashtags, postId]);
...
async function runPipeline(jobId, { brief, system, format, tone }) {
  const job = jobs.get(jobId);
  const setStep = (step, status, data = {}) => {
    job.steps[step] = { status, ...data };
  };

  try {
    // Create draft post
    const draftResult = await query(
      `INSERT INTO posts (brief, system, format, tone, status) VALUES ($1,$2,$3,$4,'draft') RETURNING id`,
      [brief, system, format, tone]
    );
    ...
    await query(
      'UPDATE posts SET headline=$1, bullets=$2, cta=$3 WHERE id=$4',
      [copy.headline, JSON.stringify(copy.bullets), copy.cta, postId]
    );
    setStep('copy', 'done', { headline: copy.headline, bullets: copy.bullets, cta: copy.cta });
    ...
    await query(
      'UPDATE posts SET caption=$1, hashtags=$2, status=$3 WHERE id=$4',
